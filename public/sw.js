  ...
  event.waitUntil(
    (async () => {
      try {
        ...
        await staticCache.addAll(STATIC_ASSETS.filter(url => 
          !url.includes('_next') // Skip Next.js assets during install as they may not exist yet
        ));
...
self.addEventListener('push', (event) => {
  if (event.data) {
    ...
    event.waitUntil(
      self.registration.showNotification(data.title || 'Vibely', options)
    );
  }
});
