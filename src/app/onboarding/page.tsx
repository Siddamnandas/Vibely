...
const steps = [
  ...
  },
  {
    icon: Lock,
    title: "Privacy First",
    description: "Your photos are analyzed locally on your device and never uploaded to our servers.",
    id: "privacy"
  }
];
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-green-400 via-primary to-secondary flex items-center justify-center p-4">
      <div className="w-full max-w-md mx-auto">
        ...
        <div className="space-y-6">
          {steps.map((step) => (
            <Card key={step.id} className="bg-white/10 backdrop-blur-lg border border-white/20 text-white rounded-3xl">
              <CardContent className="p-6 flex items-center gap-6">
                <div className={cn(
                    "w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0",
                     completedSteps.includes(step.id) ? "bg-primary text-primary-foreground" : "bg-white/20"
                )}>
                ...
                {!completedSteps.includes(step.id) && (
                  <Button size="sm" className="rounded-full bg-white text-black hover:bg-gray-200" onClick={() => handleAction(step.id)}>
