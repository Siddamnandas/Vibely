...
export default function AuthErrorPage() {
  ...
  useEffect(() => {
    ...
    setErrorDetails({
      provider:
        provider === "spotify" ? "Spotify" : provider === "apple-music" ? "Apple Music" : provider,
      error,
      description,
    });
  }, [searchParams]);
    ...
    <div className="min-h-screen bg-[#0E0F12] flex items-center justify-center px-6">
      <div className="text-center max-w-lg">
        <div className="mb-8">
          ...
          <p className="text-white/70 text-lg mb-4">Failed to connect to {errorDetails.provider}</p>
          ...
          <div className="bg-[#FF6F91]/10 border border-[#FF6F91]/20 rounded-2xl p-6 mb-6 text-left">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-white/70 text-sm leading-relaxed">{errorDetails.description}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 text-xs text-white/40">Error Code: {errorDetails.error}</div>
      </div>
    </div>
  );
}
