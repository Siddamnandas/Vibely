...
export default function AuthErrorPage({ searchParams = {} as any }: any) {
  const providerParam = (searchParams.provider as string) || "Unknown";
  const error = (searchParams.error as string) || "unknown_error";

  const provider = providerParam === "spotify" ? "Spotify" : providerParam === "apple-music" ? "Apple Music" : providerParam;

  let description = "An unexpected error occurred during authentication.";
  switch (error) {
    case "access_denied":
      description =
        "You denied access to your music library. To use Vibely, we need permission to access your music.";
      break;
    ...
    <div className="min-h-screen bg-[#0E0F12] flex items-center justify-center px-6">
      <div className="text-center max-w-lg">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-black text-white mb-2">Connection Failed</h1>

          <p className="text-white/70 text-lg mb-4">Failed to connect to {provider}</p>
        ...
        <div className="space-y-4">
          <Link
            href="/"
            className="block w-full text-center bg-gradient-to-r from-[#9FFFA2] to-[#FF6F91] text-black font-bold hover:opacity-90 rounded-full py-3"
          >
            Try Again
          </Link>
          ...
          <Link
            href="/help"
            className="block w-full text-center border border-white/20 text-white hover:bg-white/10 rounded-full py-3"
          >
            Get Help
          </Link>
        </div>

        <div className="mt-8 text-xs text-white/40">Error Code: {error}</div>
      </div>
    </div>
  );
}
