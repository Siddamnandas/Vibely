  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      try {
        ...
        if (success) {
          setStatus("success");
          setMessage(`Successfully connected to ${provider === "spotify" ? "Spotify" : "Apple Music"}!`);
    ...
    handleAuth();
  }, [provider, code, handleSpotifyCallback, handleAppleMusicCallback, router]);
  ...
  return (
    <div className="min-h-screen bg-[#0E0F12] flex items-center justify-center px-6">
      <div className="text-center max-w-md">
        ...
        {status === "error" && (
          <Button onClick={() => router.push("/")} className="bg-gradient-to-r from-[#9FFFA2] to-[#FF6F91] text-black font-bold hover:opacity-90 rounded-full px-8">
            Return to App
          </Button>
        )}
