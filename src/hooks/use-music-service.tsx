...
export function useMusicService() {
      ...
      try {
        ...
        if (state.provider === "spotify") {
          ...
          const artistIds = Array.from(
            new Set(spotifyTracks.flatMap((t) => t.artists?.map((a) => a.id) || [])),
          );
  ...
  const getPlaylistTracks = useCallback(
    async (playlistId: string): Promise<VibelyTrack[]> => {
      if (!state.provider) return [];
  ...
  const searchTracks = useCallback(
    async (query: string, limit: number = 20): Promise<VibelyTrack[]> => {
      if (!state.provider) return [];
  ...
  const connectProvider = useCallback(
    async (provider: "spotify" | "apple-music") => {
      try {
        if (provider === "spotify") {
