...
export function FullPlayer({ onClose, isVisible }: { onClose: () => void; isVisible: boolean }) {
  ...
  }, [current, currentPlaylistId, jobs, isPlaying, trackStartTime, displayCover, pendingCoverUpdate]);
  ...
  const handleSwipe = (event: any, info: PanInfo) => {
    ...
    const isSignificantSwipe = Math.abs(info.offset.x) > swipeThreshold || Math.abs(info.velocity.x) > swipeVelocityThreshold;
