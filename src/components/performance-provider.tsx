...
import { performanceMonitor, PerformanceMetrics, createPerformanceTimer } from '@/lib/performance-monitor';
      ...
      {isVisible && (
        <div className="fixed bottom-16 left-4 z-[9999] bg-black/90 text-white p-4 rounded-lg text-xs font-mono backdrop-blur-sm border border-white/20 max-w-xs">
          ...
          <div className="space-y-1">
            <div>Score: <span className="text-yellow-400">{score}/100</span></div>
            
            {metrics.LCP && (
              <div>LCP: <span className="text-blue-400">{Math.round(metrics.LCP)}ms</span></div>
            )}
            
            {metrics.FID && (
              <div>FID: <span className="text-purple-400">{Math.round(metrics.FID)}ms</span></div>
            )}
            
            {metrics.CLS && (
              <div>CLS: <span className="text-red-400">{metrics.CLS.toFixed(3)}</span></div>
            )}
            
            {metrics.FCP && (
              <div>FCP: <span className="text-green-400">{Math.round(metrics.FCP)}ms</span></div>
            )}
            
            {metrics.musicLoadTime && (
              <div>Music: <span className="text-orange-400">{Math.round(metrics.musicLoadTime)}ms</span></div>
            )}
          </div>
...
export function useComponentPerformance(componentName: string) {
  ...
  const trackInteraction = React.useCallback((interactionName: string, startTime: number) => {
