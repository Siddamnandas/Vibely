...
import { Play, Shuffle, Sparkles, PauseCircle, XCircle, RotateCcw, ChevronDown, AlertTriangle } from "lucide-react";
...
export default function PlaylistDetailOverlay({ playlistId, onClose }: { playlistId: string; onClose: () => void }) {
  ...
  const tracks = playlist.songs.map((s) => {
    ...
    return { id: s.id, title: s.title, artist: s.artist, coverUrl: updated ?? s.coverUrl, available: s.available };
  });
  ...
  const startRegen = () => {
    setShowModal(false);
    start(playlist.id, tracks.map((t) => t.id), coversMap);
  };
      ...
      <div className="fixed inset-0 z-[55] bg-[#0E0F12] text-white overflow-y-auto" ref={scrollRef}>
            ...
            <div className="absolute top-6 left-6 right-6">
              <div className="rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 p-3">
                ...
                <div className="flex items-center gap-2">
                  {job.status === 'running' ? (
                    <Button size="sm" variant="ghost" className="h-8" onClick={() => pause(playlist.id)}>
                      <PauseCircle className="h-4 w-4 mr-1" /> Pause
                    </Button>
                  ) : job.status === 'paused' ? (
                    <Button size="sm" variant="ghost" className="h-8" onClick={() => resume(playlist.id)}>
                      <Play className="h-4 w-4 mr-1" /> Resume
                    </Button>
                  ) : null}
                  <Button size="sm" variant="ghost" className="h-8 text-red-300" onClick={() => cancel(playlist.id)}>
          ...
          <div className="container mx-auto max-w-4xl px-4 py-3 flex items-center gap-3">
            ...
            <Dialog open={showModal} onOpenChange={setShowModal}>
              ...
              <DialogContent className="rounded-3xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    We’ll queue {playlist.count} songs. This may take a few minutes. You can keep listening.
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                  <Button variant="secondary" onClick={() => setShowModal(false)}>Cancel</Button>
                  <Button onClick={startRegen} className="bg-gradient-to-r from-[#9FFFA2] to-[#8FD3FF] text-black font-semibold">Start</Button>
                </DialogFooter>
              </DialogContent>
            </Dialog>
            ...
            <div className="ml-auto flex items-center gap-2">
              {job?.status === 'completed' && (
                <Badge variant="secondary" className="bg-white/10 text-white">New covers are ready ✨</Badge>
              )}
              {job && (
        ...
        {!checking && !isAuthenticated && (
          <div className="mx-4 mt-4 rounded-2xl bg-white/10 border border-white/20 p-4">
            <p className="text-white font-medium">Playback needs permission. Reconnect to {provider ?? 'your provider'}.</p>
            <p className="text-white/70 text-sm">We need to refresh your streaming connection to play songs.</p>
            <div className="mt-3">
        ...
        <div className="container mx-auto max-w-4xl px-4 py-4">
          ...
          <ul className="divide-y divide-white/10">
            {tracks.map((t, idx) => {
              ...
              const statusText = rowState?.status === 'pending'
                ? 'New cover pending'
                ...
                <li key={t.id} className="py-3" data-track-id={t.id}>
                  <div className={`flex items-center gap-3 ${disabled ? 'opacity-50' : ''}`}>
                    <div className="relative w-16 h-16 rounded-2xl overflow-hidden flex-shrink-0">
                      <Image src={rowState?.newCoverUrl ?? t.coverUrl} alt={t.title} fill className="object-cover" />
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      {disabled ? (
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="px-3 py-2 text-xs rounded-full bg-white/10">Unavailable</div>
                          </TooltipTrigger>
                          <TooltipContent>Track is unavailable in your region</TooltipContent>
                        </Tooltip>
                      ) : (
                        <Button
                          onClick={() => { setQueueWithPlaylist(playlist.id, tracks, idx); play(); }}
                          size="icon"
                      ...
                      {rowState?.status === 'updated' && (
                        <Button variant="ghost" size="sm" onClick={() => restoreTrack(playlist.id, t.id)} aria-label={`Restore cover for ${t.title}`}>
        ...
        {job?.status === 'completed' && (
          <div
            ...
            style={{ bottom: 'calc(var(--mini-height, 68px) + var(--mini-offset-bottom, 0px) + 8px)' }}
          >
            <motion.div initial={{ scale: 0.96, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 300, damping: 26 }} className="rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 p-4 shadow-lg flex items-center justify-between">
              <div>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (el instanceof HTMLElement) {
                    ...
                    setTimeout(() => el.classList.remove('ring-2','ring-[#9FFFA2]','rounded-xl'), 1200);
                  }
                }}
              >
                View changes
