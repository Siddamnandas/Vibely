...
class CoverGenerationEngine {
  ...
  generateColorPalette(track: VibelyTrack, options: GenerationOptions): [string, string] {
    ...
    const colors =
      palettes[palette][mood as keyof typeof palettes.vibrant] || palettes.vibrant.happy;
    ...
    if (processedPhoto && track.originalCoverUrl) {
      try {
        ...
        if (response.ok) {
          ...
          imageUrl =
            aiResult.generatedCoverUris[0] ||
            this.generateMockCover(track, template, processedPhoto);
        } else {
