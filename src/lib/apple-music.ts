...
class AppleMusicService {
  ...
  async authorize(): Promise<string> {
    ...
    authUrl.searchParams.append('redirect_uri', `${window.location.origin}/api/auth/apple-music/callback`);
  ...
  async exchangeCodeForToken(code: string): Promise<boolean> {
    try {
      ...
      localStorage.setItem('apple_music_expires_at', 
  ...
  private extractMoodFromGenres(genres: string[]): string {
    ...
    if (genreText.includes('electronic') || genreText.includes('dance') || genreText.includes('pop')) {
      return 'Energetic';
    } else if (genreText.includes('jazz') || genreText.includes('blues') || genreText.includes('ambient')) {
      return 'Chill';
    } else if (genreText.includes('rock') || genreText.includes('metal') || genreText.includes('punk')) {
      return 'Energetic';
