...
class PerformanceBenchmarkReportGenerator {
  ...
  private generateAnalysis(results: PerformanceMetrics[]) {
    ...
    results.forEach((result) => {
      if (result.score.rendering < 60)
        bottlenecks["Rendering Performance"] = (bottlenecks["Rendering Performance"] || 0) + 1;
      if (result.score.memory < 60)
        bottlenecks["Memory Management"] = (bottlenecks["Memory Management"] || 0) + 1;
      if (result.score.network < 60)
        bottlenecks["Network Performance"] = (bottlenecks["Network Performance"] || 0) + 1;
      if (result.score.interaction < 60)
        bottlenecks["Touch Responsiveness"] = (bottlenecks["Touch Responsiveness"] || 0) + 1;
      if (result.score.battery < 60)
        bottlenecks["Battery Efficiency"] = (bottlenecks["Battery Efficiency"] || 0) + 1;
    });
  ...
  private generateDetailedMetrics(results: PerformanceMetrics[]) {
    ...
    const totalFrames = results.reduce(
      (sum, r) => sum + (r.renderingPerformance.jankFrames || 0) + 100,
      0,
    );
    const totalJankFrames = results.reduce(
      (sum, r) => sum + (r.renderingPerformance.jankFrames || 0),
      0,
    );
    ...
    const averageTouchLatency =
      Math.round((touchLatencies.reduce((a, b) => a + b, 0) / touchLatencies.length) * 10) / 10;
    ...
    const averageCpuUsage =
      Math.round((cpuUsages.reduce((a, b) => a + b, 0) / cpuUsages.length) * 10) / 10;
...
${analysis.poorlyPerformingDevices
  .map(
    (device, index) =>
      `${index + 1}. **${device.deviceInfo.platform} ${device.deviceInfo.device}** - Score: ${device.score.overall}/100`,
  )
...
## Platform Analysis
${Object.entries(analysis.performanceTrends.byPlatform)
  .map(([platform, score]) => `- **${platform}**: ${score}/100 average`)
  .join("\n")}

## Device Category Analysis
${Object.entries(analysis.performanceTrends.byDeviceCategory)
  .map(([category, score]) => `- **${category}**: ${score}/100 average`)
  .join("\n")}
