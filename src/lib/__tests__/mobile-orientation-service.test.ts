...
describe(\"MobileOrientationService\", () => {
  ...
  describe(\"Configuration\", () => {
    test(\"updates configuration correctly\", () => {
      ...
      const setPropertyCalls = (document.documentElement.style.setProperty as jest.Mock).mock.calls.length;
      
      mobileOrientationService.forceRefresh();
      
      // Should not have added new setProperty calls
      expect((document.documentElement.style.setProperty as jest.Mock).mock.calls.length)
        .toBe(setPropertyCalls);
    });
  });
